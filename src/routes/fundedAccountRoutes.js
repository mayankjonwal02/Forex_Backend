...
router.get('/:accountId', async (req, res) => {
  try {
    const account = await fundedAccountService.getFundedAccountById(req.params.accountId);
    if (!account) {
...
router.post('/:accountId/deposit', async (req, res) => {
  try {
    ...
    const account = await fundedAccountService.depositFunds(req.params.accountId, amount);
    res.json(account);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
router.post('/:accountId/withdraw', async (req, res) => {
  try {
    const { amount } = req.body;
    const account = await fundedAccountService.withdrawFunds(req.params.accountId, amount);
    res.json(account);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
